...
4. สร้างตารางทั้งหมด — เลือกวิธีใดวิธีหนึ่ง:
   ...
   **ถ้าต้องการข้อมูลตัวอย่าง (สาขา/บริการ/หมอนวด/บัญชี login)** — โดยเฉพาะถ้าเพิ่งสร้างตารางเปล่าๆ
   แล้วเข้าเว็บเจอ "ยังไม่มีสาขาที่ใช้งานอยู่" เพราะยังไม่มีข้อมูลเลย เลือกวิธีใดวิธีหนึ่ง (ต้องรัน
   **หลัง** สร้างตารางเสร็จแล้วเท่านั้น ทั้งสองวิธีรันซ้ำได้ปลอดภัย ไม่สร้างข้อมูลซ้ำ):
   - มี Node: `npm run db:seed`
   - ไม่มี Node: copy `supabase/seed.sql` วางใน SQL Editor แล้ว Run (ข้อมูลเดียวกัน)

   ทั้งสองวิธีสร้างบัญชี login เดียวกัน: `owner@massageshop.test` / `staff@massageshop.test` /
   `nok@massageshop.test` / `waew@massageshop.test` / `oi@massageshop.test` รหัสผ่าน
   `Password123!` ทุกบัญชี (demo เท่านั้น — เปลี่ยน/ลบก่อนใช้งานจริงตาม checklist ด้านล่าง)

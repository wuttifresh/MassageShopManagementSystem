/// Shared "start a booking" reply logic for messaging-channel webhooks (LINE) — Phase 3 of the Go
/// migration plan. Deliberately channel-agnostic: it only decides *whether* an inbound text should
/// trigger a reply and *what URL/copy* that reply points at. The webhook route still owns its own
/// signature verification and its own channel-specific message format (LINE's Flex Message
/// bubble) — this module exists so that decision isn't duplicated if more channels are added.
///
...
/// nor the webhook it's used from call services/booking-core (Phase 1) yet — there's no booking
/// being created at this step, just a link being sent.
...
/// webhook has no way to know which branch a given LINE conversation is about.

import { type BookingCustomerIdentity, type CreatedBooking } from "@/lib/booking-service";
...
/// and logs the attempt. Only for `customer.type === "channel"` (LINE LIFF) — the pre-existing
/// web/LINE-Login booking flow (src/app/book/actions.ts) already sends its own confirmation and
/// is intentionally left untouched here (see the Phase 5 checkpoint notes for why). Never throws,
/// matching sendLineMessage's own contract.
export async function notifyChannelBookingConfirmed(
  ...
  const result = await sendLineMessage(customer.channelUserId, text);
